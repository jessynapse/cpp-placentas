# ==============================================================================
# 07-load-clean-corrected.R
# CPP Placental Pathology → Infantile Hemangioma
# September 2026. Corrected copy of 01-load-clean.R
#
# CHANGES FROM 01 (each marked "# CHANGED:" below):
#   1. Multiples excluded on plurality (C10 == 1), not only on PREGID
#      appearing more than once. 01 kept 26 twins whose co-twin had no
#      pathology record, so the PREGID appeared once after the merge.
#   2. Chronic hypertension includes HTN_PREG level 4 (chronic HTN with
#      superimposed preeclampsia). HTN_PREG codes: 0 normotensive,
...
#
# Original 01 decisions (unchanged unless noted above):
#   - No GA restriction (Alexa twin paper approach)
#   - Twins excluded pending CHILDID merge (Alexa returns ~June 2026)
#   - MVM dedup via slice_max (see change 4 above: all conflicts are
#     multiples, so dedup does not affect the singleton sample)
...
htn_data <- read_sas("cpp_htn_20231031.sas7bdat") %>%
  ...
  # CHANGED: level 4 (superimposed preeclampsia) is also chronic HTN.
  # Missing HTN_PREG stays NA.
  mutate(chronic_htn = if_else(is.na(HTN_PREG), NA_integer_,
                               as.integer(HTN_PREG %in% c(1, 4))))
...
# CHANGED: also require a recorded singleton birth (C10 == 1). The
# PREGID check above misses a twin whose co-twin has no pathology record.
n_before_c10 <- analytic %>%
  filter(!PREGID %in% twin_pregids, !is.na(MVM_SCORE)) %>% nrow()
...
var_label(analytic) <- list(
  ...
  chronic_htn  = "Chronic hypertension (HTN_PREG 1 or 4): 0=No, 1=Yes",
...
# CHANGED: _corrected file names so the original outputs are kept
saveRDS(analytic, "analytic_sample_corrected.RDS")
write_csv(analytic, "analytic_sample_corrected.csv", na = "")
write_csv(tibble(variable = names(var_label(analytic)),
                 label    = unlist(var_label(analytic))), "labels_corrected.csv")

cat("\nSaved: analytic_sample_corrected.RDS / .csv / labels_corrected.csv\n")
